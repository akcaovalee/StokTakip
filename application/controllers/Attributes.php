	...
	*/
	public function fetchAttributeData()
	{
		...
		foreach ($data as $key => $value) {
			...
			// button
			$buttons = '<a href="'.base_url('attributes/addvalue/'.$value['id']).'" class="btn btn-default"><i class="fa fa-plus"></i> Ürün Ekle</a> 
	...
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	if($create == true) {
        		...
        		$response['messages'] = 'Başarıyla oluşturuldu';
        	}
        	else {
	...
	public function update($id)
	{
		...
		if($id) {
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	...
	        	if($update == true) {
	        		$response['success'] = true;
	        		$response['messages'] = 'Başarıyla güncellendi!';
	        	}
	        	else {
	        		$response['success'] = false;
	...
	*/
	public function remove()
	{
		...
		if($attribute_id) {
			...
			if($delete == true) {
				$response['success'] = true;
				$response['messages'] = "Başarıyla kaldırıldı";
			}
			else {
				$response['success'] = false;
